...
function collectObjects(save) {
    ...
    const visit = (prefab, gameObject, stored) => {
        ...
        const elementData = (0, utils_1.getBehavior)(gameObject, known_behaviors_1.PrimaryElementBehavior)
            ?.templateData;
...
function describeDuplicant(gameObject) {
    const identity = (0, utils_1.getBehavior)(gameObject, known_behaviors_1.MinionIdentityBehavior)
        ?.templateData;
    ...
    const attributes = (0, utils_1.getBehavior)(gameObject, known_behaviors_1.AIAttributeLevelsBehavior)
        ?.templateData;
    const health = (0, utils_1.getBehavior)(gameObject, known_behaviors_1.HealthBehavior)?.templateData;
    return {
        name: identity?.name,
        gender: identity?.gender,
        arrivalCycle: identity?.arrivalTime != null
...
function healthStateName(state) {
    const name = typeof state === "number" ? template_enumerations_1.HealthState[state] : undefined;
